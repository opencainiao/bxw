$().ready(function() {
	...
	$("#btn_add").click(function() {
		var url_to = $.getSitePath() + '/backend/emailtype/add';
...
var data_manage_functions = {
	...
	delOne : function(data) {
		...
		var url_to = $.getSitePath() + '/backend/emailtype/' + data["_id_m"] + '/delete';
	...
	toEdit : function(data) {
		...
		var url = $.getSitePath() + '/backend/emailtype/' + data["_id_m"] + "/update";

		$.popUpWindow("编辑邮箱类型信息", url, "800px", "400px", "edit", $("#data_manage"));
	},
	/***************************************************************************
	 * 进入详细信息页面
	 * 
	...
	toDetail : function(data) {
		...
		var url = $.getSitePath() + '/backend/emailtype/' + data["_id_m"];

		$.showDetailWindow("邮箱类型信息", url, "600px", "300px");
	},
	/***************************************************************************
	 * 关闭编辑窗口
	 */
...
var data_manage = {
	...
	search : function() {
		...
		data_manage.gridsetting.url = $.getSitePath() + '/backend/emailtype/list?ts=' + new Date().getTime();
	...
	init : function() {
		...
		data_manage.gridsetting.url = $.getSitePath() + '/backend/emailtype/list';
