	...
	<script>
		$().ready(function() {
			...
			$("#btn_confirm").click(function() {
				district_functions.confirm_choose_district();
			});

			$("#btn_cancel").click(function() {
				window.top.closeChoose_districtWindow();
			});

			district_search.search();
		});

		var district_functions = {
			pageLayout : function() {
			...
			confirm_choose_district : function() {
				...
				window.top.setDistrict(item_choosed);
				window.top.closeChoose_districtWindow();
			}
		};
		...
		var district_search = {
			...
			search : function() {
				...
				var city = window.top.city.value;
				//alert(city);
					
				district_search.gridsetting.url = $.getSitePath() + '/backend/city/list_by_pid?parent_id='+ city +'&ts=' + new Date().getTime();

				$("#data").flexigrid(district_search.gridsetting);
			},
			...
			gridsetting : {
				...
				procmsg : '正在查询，请稍候 ...',
				nomsg : '没有符合条件的数据',
				onSuccess : district_functions.pageLayout,
				colModel : [ {
					display : '选择',
					hide : false,
					m_type : 'radio',
