$().ready(
		function() {
			...
			$("a", $("#plan_buttons")).click(function(e) {
				e.preventDefault();
...
function initEventInTpl(){
	...
	$("a[data-type=detail]").bind("click",function(e) {
		...
		var url = $.getSitePath() + '/front/plan/' + _id_m;
...
var data_manage_functions = {
	...
	delOne : function(data) {
		...
		var url_to = $.getSitePath() + '/front/plan/'
				+ data["_id_m"] + '/delete';
...
var data_manage = {
	...
	search : function() {
		...
		data_manage.gridsetting.url = $.getSitePath()
				+ '/front/plan/list?ts=' + new Date().getTime();
	...
	init : function() {
		...
		var url = $.getSitePath() + '/front/plan/list?ts=' + new Date().getTime();
