	...
	<script>
		$().ready(function() {
			...
			$("#btn_confirm").click(function() {
				address_type_functions.confirm_choose_unit();
			});

			$("#btn_cancel").click(function() {
				window.top.closeChooseAddresstypeWindow();
			});

			address_type_search.search();
		});

		var address_type_functions = {
			pageLayout : function() {
			...
			confirm_choose_unit : function() {
				...
				var address_type_choosed = selected[0];

				parent.setAddressType(address_type_choosed);
				window.top.closeChooseAddresstypeWindow();
				
				//parent.setAddressType(address_type_choosed);
				//parent.closeChooseAddresstypeWindow();
			}
		};

		var address_type_search = {
			...
			search : function() {
				...
				address_type_search.gridsetting.url = $.getSitePath() + '/backend/addresstype/list?ts=' + new Date().getTime();

				$("#data").flexigrid(address_type_search.gridsetting);
			},
			...
			gridsetting : {
				...
				procmsg : '正在查询，请稍候 ...',
				nomsg : '没有符合条件的数据',
				onSuccess : address_type_functions.pageLayout,
				colModel : [ {
					display : '选择',
					hide : false,
					m_type : 'radio',
