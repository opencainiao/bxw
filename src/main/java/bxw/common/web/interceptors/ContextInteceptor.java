package bxw.common.web.interceptors;
...
import bxw.common.globalhandler.PageSearchRequestUtil;
...
public class ContextInteceptor implements HandlerInterceptor {
	...
	private static final Logger logger = LogManager.getLogger(ContextInteceptor.class);
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		if (controller.indexOf(".") < 0) {
			...
			// 资源文件的访问不设置
			// 0.设置客户端信息
			Map<String, String> remoteInf = HttpServletRequestUtil.getRemoteInf(request);
			ThreadContextManager.put(Contexkeys.USERBROWSER, remoteInf.get("userbrowser"));
			ThreadContextManager.put(Contexkeys.USERAGENT, remoteInf.get("useragent"));
			ThreadContextManager.put(Contexkeys.USERBROWSER_VERSION, remoteInf.get("userbrowser_version"));
			ThreadContextManager.put(Contexkeys.USEROS, remoteInf.get("useros"));
			ThreadContextManager.put(Contexkeys.USERIP, remoteInf.get("userip"));
			...
			if (sPublicData != null) {
				ThreadContextManager.put(Contexkeys.USER, sPublicData.getLoginUser());
				ThreadContextManager.put(Contexkeys.USERID, sPublicData.getLoginUser().get_id_m());
				ThreadContextManager.put(Contexkeys.USERNAME, sPublicData.getLoginUser().getUsername());
			}

			// 2.设置分页查询条件
			PageVO pageVO = PageSearchRequestUtil.getPageInfoFromRequest(request);
			ThreadContextManager.put(Contexkeys.REQUEST_PAGE_PARAM, pageVO);
